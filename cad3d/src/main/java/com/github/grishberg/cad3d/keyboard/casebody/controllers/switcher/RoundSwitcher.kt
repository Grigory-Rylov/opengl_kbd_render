...
class RoundSwitcher(private val cfg: KeyboardConfig) : Switcher {
    ...
    override fun createSwitcher(): ModelHolder {
        val cylinder = Cylinder(5.0, Radius.fromDiameter(holeDiameter).value).rotate(Angles3d.xOnly(90.0))
        return ModelHolder(cylinder, fromModel(cylinder, Color.PINK, 20))
    }

    override fun createSwitcherHole(): Abstract3dModel {
        return Cylinder(10.0, Radius.fromDiameter(holeDiameter).value).addModel(
            Cylinder(2.0, Radius.fromDiameter(outerHoleDiameter).value).moveZ(-cfg.wallsSettings.borderThickness - 0.5)
