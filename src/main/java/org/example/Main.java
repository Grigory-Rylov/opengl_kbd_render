...
public class Main implements GLEventListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    float[] lightPosition = {0.0f, 0.6f, 1.0f, 0.0f};
...
//        window = GLWindow.create(screen, glCapabilities);
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        gl.glTranslatef(xOffset, 0.0f, -200.0f);
        ...
        for (VertexHolder vertexHolder: vertexHolderList) {
            ...
            for (int i = 0; i < vertexHolder.getVerticesCount(); i ++) {
                ...
                gl.glColor4f(vert[vertexArrayIndex++], vert[vertexArrayIndex++], vert[vertexArrayIndex++], vert[vertexArrayIndex++]);
                gl.glNormal3f(normals[normalArrayIndex++], normals[normalArrayIndex++], normals[normalArrayIndex++]);
                gl.glVertex3f(x, y, z);
            }
            gl.glEnd();
        }
    ...
    private void initializeProgram(GL2 gl) {
        String vertexShaderSource = ShaderLoader.loadShader("/shader_vertex.txt");
        String fragmentShaderSource = ShaderLoader.loadShader("/shader_fragment.txt");

        ShaderProgram shaderProgram = new ShaderProgram(gl, vertexShaderSource, fragmentShaderSource);
