...
public class Vertex {
	 ...
	 * @throws com.github.grishberg.javascad.exceptions.IllegalValueException if the coords parameter is null
	 */
	public Vertex(V3d coords, Color color) {
	...
	public int copyToArray(float[] dst, int start) {
		int i = start;
		dst[i++] = (float) coords.getX();
		dst[i++] = (float) coords.getY();
		dst[i++] = (float) coords.getZ();
		
		dst[i++] = (float) color.getRed() / 255f;
		dst[i++] = (float) color.getGreen() / 255f;
		dst[i++] = (float) color.getBlue() / 255f;
