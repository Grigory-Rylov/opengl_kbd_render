...
public class Angles3d extends Basic3dFunc<Angles3d> {
	...
	private static double simplicity(V3d temp, Normal3d normalVectorA, Normal3d normalVectorB) {
		double result = 0.0;
	...
	private static class Config {
		private final V3d coord1;
		private final V3d coord2;
		private final V3d coord3;
		...
		Config(
            V3d coord1, V3d coord2, V3d coord3,
	...
	static {
		List<V3d> coords = Arrays.asList(V3d.X, V3d.Y, V3d.Z);
	...
	public Angles3d rotate(Angles3d delta) {
		for (Config conf : CONFIGS) {
			V3d temp = conf.coord1.rotate(this).rotate(delta);
			...
			if (V3d.xOnly(1.0).rotate(this).rotate(delta)
					.rotate(zOnly(-gamma)).rotate(yOnly(-beta)).rotate(xOnly(-alpha))
					.equals(V3d.xOnly(-1.0))) {
				alpha = -(180.0+alpha);
				beta = -beta;
				gamma = 180.0+gamma;
			}
			if (V3d.yOnly(1.0).rotate(this).rotate(delta)
					.rotate(zOnly(-gamma)).rotate(yOnly(-beta)).rotate(xOnly(-alpha))
					.equals(V3d.yOnly(-1.0))) {
				alpha = -(180.0+alpha);
			}
			
			Angles3d result = new Angles3d(alpha, beta, gamma);
			
			if (Abstract3d.closeEquals(V3d.X.rotate(this).rotate(delta), V3d.X.rotate(result)) &&
				Abstract3d.closeEquals(V3d.Y.rotate(this).rotate(delta), V3d.Y.rotate(result)) &&
				Abstract3d.closeEquals(V3d.Z.rotate(this).rotate(delta), V3d.Z.rotate(result))) {
				return result;
			}
		}
	...
	public boolean equals(Object obj) {
		...
		return V3d.X.rotate(this).equals(V3d.X.rotate(other)) &&
				V3d.Y.rotate(this).equals(V3d.Y.rotate(other)) &&
				V3d.Z.rotate(this).equals(V3d.Z.rotate(other));
	}
