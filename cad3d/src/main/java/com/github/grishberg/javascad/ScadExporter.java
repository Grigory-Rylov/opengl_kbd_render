...
import com.github.grishberg.openscad.basic.Angle;
...
/**
 * Generates OpenSCAD script from Abstract3dModel tree via reflection,
...
public class ScadExporter {
    ...
    public static void export(Abstract3dModel model, String outputStl, int fn) throws Exception {
    ...
    public static String generateScript(Abstract3dModel model, int fn) {
    ...
    static List<Abstract3dModel> getChildren(Abstract3dModel model) {
        try {
            return (List<Abstract3dModel>) getField(model, "childrenModels");
        } catch (Exception e) {
            return Collections.emptyList();
        }
    }

    static void emit(StringBuilder sb, Abstract3dModel model, int depth) {
        int openBraces = 0;
        try {
            ...
            if (cls.equals("Cube")) {
                ...
                List<Abstract3dModel> models = (List<Abstract3dModel>) getField(model, "models");
                sb.append(ip).append("union() {\n");
                for (Abstract3dModel child : models) emit(sb, child, innerDepth + 1);
                sb.append(ip).append("}\n");

            } else if (cls.equals("Difference")) {
                Abstract3dModel m1 = (Abstract3dModel) getField(model, "model1");
                List<Abstract3dModel> m2 = (List<Abstract3dModel>) getField(model, "model2");
                sb.append(ip).append("difference() {\n");
                emit(sb, m1, innerDepth + 1);
                for (Abstract3dModel child : m2) emit(sb, child, innerDepth + 1);
                sb.append(ip).append("}\n");
                ...
                Abstract3dModel child = (Abstract3dModel) getField(model, "model");
                ...
                List<Abstract3dModel> children = getChildren(model);
                for (Abstract3dModel child : children) emit(sb, child, innerDepth + 1);
                sb.append(ip).append("}\n");
